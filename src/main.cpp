...
#if defined(NDEBUG)
# error "SmashCoinLite cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SmashCoinLite: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SmashCoinLite Signed Message:\n";
...
{
    // SmashCoinLite: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // SmashCoinLite: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // SmashCoinLite: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // SmashCoinLite: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SmashCoinLite: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SmashCoinLite: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Saturday 18th April 2015, First litecoin clone by Cryptcollector";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // SmashCoinLite: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SmashCoinLiteMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SmashCoinLiteMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SmashCoinLiteMiner(CWallet *pwallet)
{
    printf("SmashCoinLiteMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("smashcoinlite-miner");
    ...
    try { loop {
        ...
        printf("Running SmashCoinLiteMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SmashCoinLiteMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SmashCoinLiteMiner, pwallet));
}
